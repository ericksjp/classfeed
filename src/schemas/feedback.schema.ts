...
const FeedbackInput = z
    .object({
        lessonId: z.string({ message: "lessonId is required" }).uuid({ message: "Invalid lesson ID" }),

        studentId: z.string({ message: "studentId is required" }).uuid({ message: "Invalid student ID" }),
