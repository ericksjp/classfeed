...
export function isUserPartOfClass(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
async function checkUserInClass(req: Request, res: Response, next: NextFunction, role: "teacher" | "student" | "both") {
  ...
  if (role === "both") {
    ...
    if (!teacherClass && !studentClass) {
      return next(new AuthorizationError(403, "You are neither the teacher nor a student in this class", "ERR_FORBIDDEN"));
    }
  }

  if (role === "teacher") {
    teacherClass = await teacherQuery()
    if (!teacherClass) return next(new AuthorizationError(403, "You are not the teacher for this class", "ERR_FORBIDDEN"));
  }

  if (role === "student") {
    studentClass = await studentQuery()
    if (!studentClass) return next(new AuthorizationError(403, "You are not enrolled as a student in this class", "ERR_FORBIDDEN"));
  }
